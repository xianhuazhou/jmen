...
(function(){
    ...
    function printHelp() {
        ...
        help("  --scss-path| -s\tscss path (for *.scss files only)");
        process.exit(0);
    }
    ...
    var params = (function(args){
        ...
        for (var index = 0; index < args.length; index++) { 
            ...
            parseParam(index, 'scssPath', /(\-\-scss\-path|\-s)/);
        }

        params.encoding = params.encoding || 'utf8';
        params.scssPath = params.scssPath || 'scss';
    ...
    var JMen = {
        ...
        // Parse scss content
        parseScssData: function(filePath) {
            var self = this;
            exec(params.scssPath + " " + filePath, function(error, stdout, stderr) {
                if (error !== null) {
                    puts("Error: " + error);
                    throw "Can't find the \"scss\" command in your path, please install scss from http://scss-lang.com/.";
                }
                return self.result[filePath] = stdout;
            });
        },
        ...
        parseData: function(filePath, fileExt, data) {
            ...
            switch (fileExt) {
                case '.coffee':
                    ...
                    break;
                case '.scss': 
                    self.parseScssData(filePath, false);
                    break;
                case '.css':
                case '.less':
        ...
        checkProgress: function() {
            ...
            if (Object.keys(self.result).length >= self.files.length) {
        ...
        // Need optimize this
        // Including files at top in 2+ levels doesn't work well ...
